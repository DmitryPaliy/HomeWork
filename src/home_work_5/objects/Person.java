...
//	4.2 Генерировать рандомную строку (состоящую из английских букв)
...
//		К примеру: есть список животных [{age: 10, nick: "Анатолий"}, {age: 10, nick: "Игнат"}, {age: 1, nick:
//		"Люцифер"}, {age: 10, nick: "Ярик"}].
//		После сортировки должно получиться: [{{age: 1, nick: "Люцифер"}, {age: 10, nick: "Анатолий"}, {age: 10, nick:
//		"Игнат"}, {age: 10, nick: "Ярик"}].
//		Так как сначала сортируется по возрасту в порядке возрастания, а потом в рамках группы (пример: age = 10),
...
//	"Операция: ХХХХ. Заняла YYYY мс". Для замера использовать метод из jdk System.currentTimeMillis():
...
public class Person {
    private final String nick; //псевдоним пользователя
    private final String password;// пароль (от 5 до 10 символов)
    ...
    public String toString() {
        return "Person{" + "ник " + nick + ", пароль " + password + ", имя " + name + '}';
    }
}
